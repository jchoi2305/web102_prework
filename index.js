...
raisedCard.innerHTML = `<p>$${totalPledged.toLocaleString()}`;

// -> displaying the total # of games in the total games card
// grab number of games card and set its inner HTML
const gamesCard = document.getElementById("num-games");

// displaying the length of GAMES_JSON because it = number of games
...
function filterUnfundedOnly() {
    ...
    let goalUnmetGames = GAMES_JSON.filter( (game) => {
        return game.pledged < game.goal;
    });

    // use the function we previously created to add the unfunded games to the DOM
    addGamesToPage(goalUnmetGames);
    // console.log(goalUnmetGames);
}
// filterUnfundedOnly();
...
function filterFundedOnly() {
    ...
    let goalMetGames = GAMES_JSON.filter( (game) => {
        return game.pledged >= game.goal;
    });
...
unfundedBtn.addEventListener('click', filterUnfundedOnly); // unfunded
fundedBtn.addEventListener('click', filterFundedOnly); // funded
allBtn.addEventListener('click', showAllGames); // all games
...
// using filter, we're grabbing the list of unfunded games by calling the function made in challenge 6
...
// source/aid: https://www.w3schools.com/js/js_htmldom_nodes.asp & https://developer.mozilla.org/en-US/docs/Web/API/Document/createElement 
const newElem = document.createElement("div");
newElem.innerHTML = `<p>${displayStr}</p>`;
